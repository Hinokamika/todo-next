...
export default function TodoAppClient({ initialTodos = [] }: TodoAppClientProps) {
    ...
    const completionPercentage = todos.length > 0 ? Math.round((todos.filter((task) => task.completed).length / todos.length) * 100) : 0;
    ...
    return (
        <div className="min-h-screen px-20 py-5 justify-start font-sans bg-green-950">
            ...
            <div className="max-w-screen flex justify-between text-white">
                <p className="text-gray-500">You have {completedCount} remaining</p>
                <div className="">
                    <div className="flex gap-32 mb-2">
                        <p className="font-bold">Daily Goal</p>
                        <p className="font-bold text-green-600">{completionPercentage}%</p>
                    </div>
                    <Progress
            ...
            <Tabs defaultValue="All">
                <TabsList className="my-2 bg-[#29382f] rounded-full">
                    <TabsTrigger value="All" className="rounded-full font-bold transition-all px-4 py-2">
                        All
                    </TabsTrigger>
                    <TabsTrigger value="Active" className="rounded-full font-bold transition-all px-4 py-2">
                        Active
                    </TabsTrigger>
                    <TabsTrigger value="Completed" className="rounded-full font-bold transition-all px-4 py-2">
                        Completed
                    </TabsTrigger>
                </TabsList>
                <TabsContent value="All">
                    {todos.map((task) => (
                        <NoteCards
                            ...
                            site_content={task.site_content}
